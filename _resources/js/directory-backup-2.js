  ...
  filterTypes.forEach(type => {
    if (filters[type].length) {
      filteredItems = filteredItems.filter(item => {
        ...
        // Check if parent checkbox is checked for this type
        ...
        if (parentChecked) {
          // Only match items whose parent title matches the checked parent filter
          ...
          // If parent not checked, match by children for this type
          return filters[type].some(filterValue => {
            const normalizedFilter = filterValue
              .trim()
              .replace(/[^a-zA-Z0-9_]/g, "")
